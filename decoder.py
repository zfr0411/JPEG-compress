...
def main():
    print("请在命令行运行此程序")
    TxtName = raw_input("Choose txt you want decode : ")
    dc, ac, tables, blocks_count = read_image_file(TxtName)
    ...
    im_ = Image.open(base+".jpg")
    im_mat = np.array(im_)
    width, height = im_mat.shape[0], im_mat.shape[1]
    im_resize = image.resize((height, width))
    im_resize.show()
    imagename = base+"-JPEG_Compressed.jpg"

    im_resize.save(imagename)
